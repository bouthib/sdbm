...
---------------------------------------------
---------------------------------------------
--  V E R S I O N   0 . 2 3  -   B e t a   --
---------------------------------------------
---------------------------------------------


ALTER TABLE SDBM.PARAMETRE
   ADD LANGUE CHAR(2);
...
'
SELECT ''N/A''
      ,''Attention : '' || COUNT(1) || '' nouveau(x) enregistrement(s) dans le journal SDBM pour la dernière heure.''
  FROM SDBM.JOURNAL
 WHERE DH_JOURNAL > SYSDATE - 65/1440
   AND NIVEAU   NOT IN (''INFO'',''CONFIG'')
 HAVING COUNT(1) > 0
'
WHERE TYPE_CIBLE      = 'BD'
  AND SOUS_TYPE_CIBLE = 'OR'
  AND NOM_EVENEMENT   = 'SDBM.JOURNAL';
...
UPDATE SDBM.EVENEMENT
   SET NOM_EVENEMENT = DECODE(NOM_EVENEMENT
                             ,'SDBM.JOURNAL','SDBM.ERROR'
                             ,'SDBM.TACHE'  ,'SDBM.JOB'
                             ,'SDBM.AGENT'  ,'SDBM.SDBMAGT'
                             ,'SDBM.SERVEUR','SDBM.SDBMSRV'
                             ,'MIROIR'      ,'MIRRORING'
                             )
 WHERE TYPE_CIBLE      = 'BD'
   AND NOM_EVENEMENT   IN ('SDBM.JOURNAL','SDBM.TACHE','SDBM.AGENT','SDBM.SERVEUR','MIROIR');

UPDATE SDBM.EVENEMENT_CIBLE
   SET NOM_EVENEMENT = DECODE(NOM_EVENEMENT
                             ,'SDBM.JOURNAL','SDBM.ERROR'
                             ,'SDBM.TACHE'  ,'SDBM.JOB'
                             ,'SDBM.AGENT'  ,'SDBM.SDBMAGT'
...
UPDATE SDBM.HIST_EVENEMENT_CIBLE
   SET NOM_EVENEMENT = DECODE(NOM_EVENEMENT
                             ,'SDBM.JOURNAL','SDBM.ERROR'
                             ,'SDBM.TACHE'  ,'SDBM.JOB'
                             ,'SDBM.AGENT'  ,'SDBM.SDBMAGT'
...
ALTER TABLE SDBM.EVENEMENT_CIBLE
   ADD CONSTRAINT EVC_FK_EVENEMENT
      FOREIGN KEY (TYPE_CIBLE, SOUS_TYPE_CIBLE, NOM_EVENEMENT) REFERENCES SDBM.EVENEMENT (TYPE_CIBLE,SOUS_TYPE_CIBLE,NOM_EVENEMENT) ON DELETE CASCADE;



-> _DEV\_Installation OS & Oracle XE\Windows\sdbm_setup-d.sql
-> _DEV\_serveur\sdbm.server.linux.x86\sdbmsrv\sdbmsrv.init.sql
...
(
  ...
  ,CHAINE_FR            VARCHAR2(4000)                   NOT NULL
  ,CHAINE_AN            VARCHAR2(4000)                   NOT NULL
  ,COMMENTAIRE_FR       VARCHAR2(4000)
  ,COMMENTAIRE_AN       VARCHAR2(4000)
)
...
ALTER TABLE EVENEMENT_DEFAUT_TRADUCTION
   ADD CONSTRAINT EVEDT_PK_EVENEMENT PRIMARY KEY (TYPE_CIBLE, SOUS_TYPE_CIBLE, NOM_EVENEMENT)
      USING INDEX
      TABLESPACE SDBM_DATA;
